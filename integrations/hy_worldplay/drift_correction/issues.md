...
| 6 | **corrgate025 dial-down eval (owner arm 2026-07-24)** — v2c2 at α*(t)×0.25 vs corrgate050 vs base on bridge (default 3 poses) + castle/flags + person card cells, seed 5042, 24 chunks. CAVEAT: the original castle/e_person ad-hoc cell poses/prompts were not persisted; those two cells were regenerated as matched within-run triples (seed image = frame 0 of the owner's cells; documented poses/prompts in the eval script) — absolute numbers not comparable to eval_v2c2 tables | **Pre-stated bars FAIL**: bridge Δ 1.53 vs base 1.18 (NO cut; 050 cuts 52% — retention bar ≥60% of 050's missed by a wide margin) · castle Δ 5.59 vs base 2.39 (worse than 050's 4.35) with visible saturation runaway in late frames (`eval_g025_castle/g025_recheck/castle_stack.png`; sharpness check inconclusive — reconstructed pose ends against a wall) · person Δ 6.70 between base 7.45 and 050's 4.39. Only win: dynamics preserved (bridge 24.6 ≈ base 20.9; castle 49.4 vs 050's 31.7) | **OWNER EYEBALL (2026-07-24): flat ×0.25 FAILS as a replacement** — better details than corrgate050 BUT color saturation drifts like base (the flat dial is too weak for HY's dominant drift mode); matches the instrument bridge-Δ fail. Follow-up arm RUNNING: **t-split gain schedule** gain(t) = 0.5 on the two high-t solver steps / 0.25 low-t (`corrtsplit_a`; second point `corrtsplit_b` = 0.5,0.5,0.5,0.25) — rationale: saturation correction is systematic and lives at high t (α* 0.81@t1000), detail damage is injected by the non-systematic low-t share (α* ~0.53), same decomposition as the pulse investigation. Bars (pre-stated): sat_drift ≈ corrgate050's (≤ ~0.05) · castle detail ≥ flat-g025's (stacks) · bridge Δ cut ≥ ~50% · dynamics no worse than −23%. Config-only if accepted: the runner needs `drift_corrector_gain` to accept a per-step schedule (currently scalar) — flagged. **T-split results (2026-07-24): MIXED vs bars** — bridge sat_drift 0.011 (a) / 0.014 (b) = best of ALL arms incl. 050's 0.037 (sat bar PASS, the high-t rationale confirmed) and dynamics fine (22.3 ≈ base), BUT bridge Δ 2.02/1.66 vs base 1.18 — the Δ-cut bar FAILS (inversion on record: 050's MUSIQ-Δ cut is apparently carried by its LOW-t share, not high-t). Person cell: tsplit_a is the best arm overall (Δ 3.95 < 050's 4.39, MUSIQ 64.6, sharpness 1.37). Castle: all corrector arms remain worse than base (stylized-content gap, issue #5). NOT a config change per bars; owner eyeball on `tsplit_recheck/` stacks + sbs decides | `outputs/eval_g025_{bridge,castle,person}/` + sbs | — | owner eyeball |
