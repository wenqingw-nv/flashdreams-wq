...
| 5 | **Person/stylized-content generalization** — OOD card-image cells (2026-07-22): corrgate050 = owner-verdict best overall on new dynamic scenes, holds bridge profile on strafe (Δ −97%, dyn ≈ base) and yaw (no anchoring), BUT people blur/morph vs base in person-walking content (no pulse), and stylized scenes (painterly snow, fantasy castle) regress on Δ/MUSIQ while trained axes (sat, sharpness) improve — corrector inherits its training content régime (test.png seeds contain no humans) | fix in flight: content-diversification DAgger round (card-image + person seeds pool, continue-train v2 at low LR); kill bar: person-scene quality improves without losing the bridge profile | **OPEN** — owner-confirmed 2026-07-22 | `outputs/eval_ood_*/` + sbs | DAgger round | ~1 d |
