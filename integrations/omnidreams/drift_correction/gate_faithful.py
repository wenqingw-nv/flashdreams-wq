...
def main() -> None:
    ...
    agg = {i: {"alphas": [], "alphas_ub": [], "rels": [], "rels_x0": []} for i in range(n_steps)}
    ...
    for clip_path in clips:
        ...
        for k in PROBE_CHUNKS:
                ...
                preds[name] = {
                    t_idx: [
                        predict_v(
                            ...
                            hdmap=hdmap_k,
                            z_t=z,
                            timestep=timesteps[t_idx].to(device=device, dtype=dtype),
                        )
                        for z in z_list
                    ]
            ...
            for t_idx in range(n_steps):
                ...
                x0_norm = (
                    torch.stack(
                        [z.float() - sig * v for z, v in zip(z_ts[t_idx], preds["gen"][t_idx])]
                ...
                line.append(
                    f"t={int(timesteps[t_idx]):4d} a*={a:.3f}/{a_ub:.3f}"
    ...
    print(f"cells with unbiased alpha* >= 0.7: {frac:.0%} | mean rel_v gap {mean_rel:.4f}")
    if mean_rel < 0.01:
