...
learns a *loop prior* (self-copying / novel-content suppression; owner
verdict 2026-07-24, ``issues.md`` #1/#2). v4 removes revisits entirely:
...
- **Clean counterpart**: every :data:`SEG_CHUNKS` chunks the build forks a
  ...
  any training (see issues.md, owner GO 2026-07-24).
    ...
    for c, item in enumerate(inputs):
        ...
        for s, c_s in enumerate(FORK_STARTS):
            ...
            cache_s = pipe.initialize_cache_from_embeddings(
                text_embeddings=emb_s["text_embeddings"],
                image_embeddings=emb_s["image_embeddings"],
            )
            ...
            fork_latents.append(
                [sn.clean_latent.detach().cpu() for sn in snaps_s[1:]]
            )
            if c == 0:
                mp4 = OUT_DIR / f"clip_{c:02d}_fork{s}.mp4"
                OUT_DIR.mkdir(parents=True, exist_ok=True)
                _write_video(video_s[0, 0].permute(0, 2, 3, 1), mp4, fps=30)
