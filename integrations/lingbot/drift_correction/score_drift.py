...
"""Drift + guard scoring for LingBot eval rollouts (Delta-MUSIQ, dynamics, strips).

Per MP4 (flat ``{scene}_{traj}_{config}.mp4`` layout from
``eval_rollouts.py``): per-frame MUSIQ (pyiqa) at a fixed frame stride,
Delta-drift = MUSIQ(first 20%) - MUSIQ(all) (BAgger's metric), a RAFT
mean-flow dynamic degree (guard: a corrector that freezes motion "wins"
every consistency metric), saturation drift, sharpness, the reference
protocol metrics (DINO latesim anchoring, lag-2s identity, full-rate cut
count, chunk-seam ratios), and a 10-frame contact strip PNG. Aggregates
per config across scenes into ``OUT/scores.json`` and prints a compact
config-vs-base table.

pyiqa / timm are not in the project venv; run under the df-gb300 conda
python from the flashdreams repo root::

    /localhome/local-wenqingw/miniconda3/envs/df-gb300/bin/python \
        integrations/lingbot/drift_correction/score_drift.py
"""
...
EVAL_DIR = Path(
    os.environ.get("OUT", "integrations/lingbot/drift_correction/outputs/eval_v1")
)
FRAME_STRIDE = int(os.environ.get("FRAME_STRIDE", "12"))
"""Score every 12th frame (one per AR chunk at the decoded rate)."""
...
def sharpness_curve(frames: np.ndarray) -> list[float]:
    ...
    loss that MUSIQ under-penalizes (observed on the HY v1-pilot rollouts)."""
...
"""Decoded frames per AR chunk after the first: LingBot's temporal VAE maps
the 3-latent seed chunk to 9 frames, then 12 per chunk (477 = 9 + 39*12 at
40 chunks), so chunk boundaries sit at frame ``9 + 12k``."""

FIRST_CHUNK_FRAMES = int(os.environ.get("FIRST_CHUNK_FRAMES", "9"))
...
def seam_motion_ratio(frames: np.ndarray) -> float:
    ...
    (boundaries at frame ``9 + 12k``; seed chunk excluded) and returns
    mean(boundary phases 0-1) / mean(interior phases 3-8). ~1.0 = motion
...
def seam_sharpness_ratio(frames: np.ndarray) -> float:
    ...
    cadence (chunk-initial frames at ``9 + 12k``; seed chunk excluded):
    mean(phases 0-1) / mean(phases 4-10). Well below 1 = post-boundary blur
...
def cut_count(frames: np.ndarray) -> int:
    ...
    ``max(0.10, mu + 5*sigma)`` (reference convention) — catches
    content-level jump-cuts at the rolling-window handoff."""
...
def dynamic_degree(frames: np.ndarray, raft, device) -> float:
    ...
    for a, b in zip(frames[:-1], frames[1:]):
        ...
        ta = F.interpolate(ta, size=(464, 832), mode="bilinear") / 127.5 - 1
        tb = F.interpolate(tb, size=(464, 832), mode="bilinear") / 127.5 - 1
        flow = raft(ta, tb)[-1]
...
def main() -> None:
    ...
    # Flat eval_rollouts.py layout: config = the stem's last ``_`` field.
    mp4s = sorted(EVAL_DIR.glob("*.mp4"))
    assert mp4s, f"no MP4s under {EVAL_DIR}; run eval_rollouts.py first"
    by_config: dict[str, list[Path]] = {}
    for mp4 in mp4s:
        by_config.setdefault(mp4.stem.rsplit("_", 1)[-1], []).append(mp4)
    configs = sorted(by_config, key=lambda c: (c != "base", c))

    results: dict[str, dict] = {}
    for config in configs:
        per_video = {}
        for mp4 in by_config[config]:
            cell = mp4.stem.rsplit("_", 1)[0]
            ...
            v = per_video[cell]
            print(
                f"{config}/{cell}: MUSIQ {np.mean(curve):.1f} "
    ...
    print("\n================ closed-loop drift eval (LingBot) ================")
    print(
